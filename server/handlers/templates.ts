...
export async function handleListTemplates(jwt: string, namespace: string): Promise<Response> {
  try {
    const k8sClient = await reuseOrCreateUserK8sClient(jwt);
    const result = await discoverAcrossNamespaces<K8sWorkspaceTemplate>(k8sClient, TEMPLATE_PLURAL, namespace, serverConfig.sharedNamespace);
    ...
    return jsonResponse({ items, access: result.access, namespaces: { own: namespace, shared: serverConfig.sharedNamespace } });
  } catch (error) {
